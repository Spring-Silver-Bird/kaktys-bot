...
from kaktus_data import BOT_TOKEN
...
async def process_numbers_answer(message: Message):
    if users[message.from_user.id]['in_game']:
        if int(message.text) == users[message.from_user.id]['secret_number']:
            ...
            attempt = users[message.from_user.id]['attempts']
            await message.answer(f'Мое число меньше. Количество оставшихся попыток: {attempt}')
        elif int(message.text) < users[message.from_user.id]['secret_number']:
            users[message.from_user.id]['attempts'] -= 1
            attempt = users[message.from_user.id]['attempts']
            await message.answer(f'Мое число больше. Количество оставшихся попыток: {attempt}')
